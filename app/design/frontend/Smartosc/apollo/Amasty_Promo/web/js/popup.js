define([
    ...
    $.widget('mage.ampromoPopup', {
         ...
         */
        _create: function () {
            let products = $.extend({}, this.options.products);
        ...
        _initHandles: function () {
            ...
            $(document).on('reloadPrice', function (item) {
                let products = $.extend({}, self.options.products);
         ...
         */
        _loadItems: function () {
            let onSuccess = this._success.bind(this),
                config = {
        ...
        _success: function (response) {
            let isReload = !_.isEmpty(this.response) &&
        ...
        _isDragEventInit: function () {
            let slickSettingsCurrent = this.gallery[0].slick.options,
         ...
         */
        sendForm: function () {
            let formData = this._prepareFormData(),
        ...
        initOneByOneProductAdd: function () {
            let onInitHandlersOneProductAdd = this.initHandlersOneProductAdd.bind(this);
        ...
        _updateProductLeftQty: function () {
            ...
            $.each(this.options.products, function (ruleId, rulesData) {
                ...
                $.each(rulesData.sku, function (key, value) {
                    let productDomBySku = self.getProductDomBySku(key),
                        qtyInput,
                        qtyInt = +value.qty;
        ...
        updateValues: function (newQty, productSku, ruleId, ruleType, elem) {
            ...
            $.each(this.options.products, function (itemRuleId, ruleData) {
                let allSkuForRule = Object.keys(ruleData.sku),
        ...
        getSumQtysByRuleId: function () {
            ...
            $.each($('[data-am-js=ampromo-qty-input]'), function () {
                let itemRuleId = $(this).attr('data-rule'),
                    value = $(this).val(),
        ...
        _calcWidthTitle: function () {
            let itemsCountReal = +this.gallery.data('count'),
        ...
        _toggleMessage: function () {
            let isToggle = this._isToggleMessage();
        ...
        checkboxState: function (elem) {
            let product = this.getProductDomByElem(elem),
                selectInput = product
