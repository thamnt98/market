...
define([
    ...
    $.widget('mage.SwatchRenderer', {
        ...
        _UpdatePrice: function () {
            ...
            //update badge discount amoun
            this._updateBadgeDiscount(result.oldPrice.amount, result.finalPrice.amount);
        },
        ...
        _getOrderProductWithMinPrice: function (orderItem) {
            ...
            var attributeIdVal = optionAttribute[orderItem].id,
                priceStatus = this.options.jsonConfig.priceProductStatus,
                priceState;
            _.each(optionPrices, function (optionPrice, key) {
                optionFinalPrice = parseFloat(optionPrice.finalPrice.amount);
                priceState = parseInt(priceStatus[key].priceStatus);
                if(priceState >1){
                    return;
                }
                if (i === 0 || ((optionFinalPrice < optionMinPrice) && (priceState < 2))) {
                    optionMinPrice = optionFinalPrice;
                    order = key;
                }
                i = i + 1;
        ...
        /**
         * Update badge discount when one of the options has selected.
         *
         * @param {*} oldPrice, finalPrice
         * @private
         */
        _updateBadgeDiscount: function (oldPrice, finalPrice) {
            var discPercentEl = $('.disc-percent');

            discPercentEl.css("display","none");
            ...
            if (oldPrice != '' && finalPrice != '' && finalPrice < oldPrice) {
                ...
                discPercentEl.css("display","inline-block");
                discPercentEl.parent().removeClass('no-percent-discount');
                originalPriceFormated = priceUtils.formatPrice(
                    oldPrice,
