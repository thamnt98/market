...
class InstallSchema implements InstallSchemaInterface
{
    ...
    public function install(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        $this->integrationCustomerCentralTable($setup);
        $this->updateIntegrationCustomerCentralTable($setup); //Update type data field central_id, because length of type data is not enough
    ...
    public function integrationJobTable($setup)
    {
        ...
        $tableName = $setup->getTable(\Trans\IntegrationCustomer\Api\Data\IntegrationJobInterface::TABLE_NAME);
    ...
    public function integrationDataValueTable($setup)
    {
        ...
        // Get tutorial_simplenews table
        $tableName = $setup->getTable(\Trans\IntegrationCustomer\Api\Data\IntegrationDataValueInterface::TABLE_NAME);
        // Check if the table already exists
        if ($setup->getConnection()->isTableExists($tableName) != true) {
            // Create tutorial_simplenews table
            $table = $setup->getConnection()
    ...
    /**
     * Create table Integration Customer Central
     * @param $installer
     */
    public function integrationCustomerCentralTable($setup) {

        // Get tutorial_simplenews table
        $tableName = $setup->getTable(IntegrationCustomerCentralInterface::TABLE_NAME);
        // Check if the table already exists
        if ($setup->getConnection()->isTableExists($tableName) != true) {
            // Create tutorial_simplenews table
            $table = $setup->getConnection()
                ->newTable($tableName)
                ->addColumn(
                    IntegrationCustomerCentralInterface::ID,
                    Table::TYPE_INTEGER,
                    null,
                    [
                        'identity' => true,
                    ...
                    ucfirst(IntegrationCustomerCentralInterface::ID)
                )
                ->addColumn(
                    IntegrationCustomerCentralInterface::CUST_ID,
                    Table::TYPE_INTEGER,
                    null,
                    ['nullable' => false, 'default' => 0],
                    ucfirst(IntegrationCustomerCentralInterface::CUST_ID)
                )
                ->addColumn(
                    IntegrationCustomerCentralInterface::CENTRAL_ID,
                    Table::TYPE_INTEGER,
                    null,
                    ['nullable' => false, 'default' => 0],
                    ucfirst(IntegrationCustomerCentralInterface::CENTRAL_ID)
                )
                ->addColumn(
                    IntegrationCustomerCentralInterface::STATUS,
                    Table::TYPE_SMALLINT,
                    null,
                    ['nullable' => false, 'default' => 1],
                    ucfirst(IntegrationCustomerCentralInterface::STATUS)
                )
                ->addColumn(
                    IntegrationChannelInterface::CREATED_AT,
                    Table::TYPE_TIMESTAMP,
                ...
                ->setComment('Integration Customer Centralize')
                ->setOption('type', 'InnoDB')
                ->setOption('charset', 'utf8');
            $setup->getConnection()->createTable($table);
        }
    ...
    /**
     * Update table Integration Customer Central for type data field central_id, because length of type data is not enough
     * @param $installer
     */
    public function updateIntegrationCustomerCentralTable($setup)
    {
