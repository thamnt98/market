...
use \Trans\IntegrationCatalogPrice\Api\Data\IntegrationJobInterfaceFactory ;
use \Trans\IntegrationCatalogPrice\Api\Data\IntegrationJobSearchResultInterface;
use \Trans\IntegrationCatalogPrice\Api\Data\IntegrationJobSearchResultInterfaceFactory;
use \Trans\IntegrationCatalogPrice\Api\IntegrationJobRepositoryInterface;

use \Trans\IntegrationCatalogPrice\Model\ResourceModel\IntegrationJob\CollectionFactory;
use \Trans\IntegrationCatalogPrice\Model\ResourceModel\IntegrationJob\Collection;
...
class IntegrationJobRepository implements IntegrationJobRepositoryInterface
{
     ...
     * {@inheritdoc}
     */
    public function getByMdIdLastComplete($mdId)
    {
        $data=NULL;
        ...
        if($collection->getSize()) {
            ...
            throw new StateException(
                __('No last complete job data found.')
            );
        }
    ...
    public function getLastCompleteJobBatch($param)
    {
        ...
        if($collection->getSize()){
            ...
            throw new StateException(__('No catalog price job data found.'));
        }
    ...
    public function getByMdIdFirstItem($mdId,$status=IntegrationJobInterface::STATUS_WAITING)
    {
        ...
        $collection->addFieldToFilter(IntegrationJobInterface::STATUS,$status);
        $collection->setPageSize(1);
        $collection->setOrder(IntegrationJobInterface::ID, 'ASC');
