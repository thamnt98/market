...
class IntegrationCheckUpdates implements IntegrationCheckUpdatesInterface
{
    ...
    protected function retryCron($instance){
        ...
        try{
            $jobData = $this->jobRepository->getById($instance->getId());
            ...
            $jobData->setHit($totalHit);
            $jobData->setStatus($status);

            $this->jobRepository->save($jobData);
    ...
    public function checkReadyJobs($channel){
        ...
        try {
            ...
            $result = $this->jobRepository->getByMdIdWithStatus($channel['method']->getId(),IntegrationJobInterface::STATUS_READY);
            if(!$result->getSize()){
                throw new StateException(
                    __("Not Exist Job")
                );
