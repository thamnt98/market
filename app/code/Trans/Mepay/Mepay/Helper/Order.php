...
class Order extends AbstractHelper
{
    ...
    {
        $result = $this->_getLastCustomer($customerId);
        $data = $this->_getLastCustomer($customerId, $result['reference_number']);
    ...
    public function _getLastCustomer(int $customerId, $referenceNumber = null)
    {
        $connection = $this->orderResource->getConnection();
        $table = $connection->getTableName('sales_order');
        $query = $connection->select();
        if ($referenceNumber) {
            $result = $query
                ->from($table, ['*'])
                ->where('customer_id = "'.$customerId.'" AND reference_number = "'.$referenceNumber.'"')
