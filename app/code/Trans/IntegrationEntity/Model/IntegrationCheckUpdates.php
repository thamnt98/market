...
class IntegrationCheckUpdates implements IntegrationCheckUpdatesInterface
{
     ...
     * @throws StateException
     */
    public function checkMultiSaveOnProgressJob($channel){
        if(!isset($channel['method'])){
            throw new StateException(
                __(IntegrationCommonInterface::MSG_METHOD_NOTAVAILABLE)
            );
        ...
        try {
            $collection = $this->jobRepository->getByMdIdMultiWithStatus($channel['method'],IntegrationJobInterface::STATUS_PROGRESS_CATEGORY);
     ...
     * @throws StateException
     */
    public function prepareJobsDataProperRespWithKey($channel=[],$response=[]){
    ...
    public function save($data=[]){
        ...
        for ($i = 0; $i < $data['job']; $i++){
            ...
            $result[$i][IntegrationJobInterface::OFFSET] = $offset;
            $this->jobRepository->saveJobs($result[$i]);
            $offset = $offset + $data[IntegrationJobInterface::LIMIT];
        }
