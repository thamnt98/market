...
class Bundle extends \Wizkunde\ConfigurableBundle\Helper\Bundle
{
    ...
    public function __construct(
        ...
        parent::__construct($context, $productConfiguration, $pricingHelper, $escaper, $productModel, $eavConfig,
            $productRepository, $checkoutSession, $scopeConfig);
    }
    ...
    private function buildSelectionData($bundleOption, $bundleSelection, ItemInterface $item)
    {
        ...
        foreach ($item->getChildren() as $childItem) {
            ...
            if ($selectionId == $bundleSelection->getSelectionId()) {
                ...
                $returnData .= '<span class="product-item-cart-info">' .
                    '<span style="line-height: 1.1; font-size: 1.2rem; margin:2px 0 20px 0;display: block;">';
                if ($bundleSelection->getTypeId() !== 'configurable') {
            ...
            foreach ($attributeOptions as $optionData) {
                if ($optionData['value'] == $value) {
                    $optionHtml .=
                        ...
                        '<span>' . $attribute->getStoreLabel() . ': ' . $optionData['label'] . '</span>' .
                        '</span>';
                }
            }
        }
