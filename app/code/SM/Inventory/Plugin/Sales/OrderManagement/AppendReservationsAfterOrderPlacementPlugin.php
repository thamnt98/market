 ...
 * @package SM\Inventory\Plugin\Sales\OrderManagement
...
namespace SM\Inventory\Plugin\Sales\OrderManagement;
...
class AppendReservationsAfterOrderPlacementPlugin
{
    ...
    {
        if ($order->getIsVirtual() || $this->checkIsParentOrder($order)) {
            return $order;
        }
     ...
     * @return bool
     */
    protected function checkIsParentOrder($order)
    {
        return (empty($order->getData('is_parent'))
                || $order->getData('is_parent') == 1)
            && empty($order->getData('parent_order'));
    }
}
