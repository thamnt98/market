...
namespace SM\CustomPrice\Model\Import;
use Exception;
...
class StoreLocation extends AbstractEntity
{
    const ENTITY_CODE = 'store_location';
    const TABLE = 'omni_location_mapping';
    ...
    {
        $name = $rowData['omni_code'] ?? '';
        $duration = (int) $rowData['district_id'] ?? 0;

        if (!$name) {
            $this->addRowError('OmniCodeIsRequired', $rowNum);
        }

        if (!$duration) {
            $this->addRowError('DistrictIdIsRequired', $rowNum);
        }
    ...
    private function initMessageTemplates()
    {
        $this->addMessageTemplate(
            'OmniCodeIsRequired',
            __('The Omni code cannot be empty.')
        );
        $this->addMessageTemplate(
            'DistrictIdIsRequired',
            __('The District Id cannot be empty.')
        );
    }
