...
abstract class AbstractLookbook extends \Magento\Framework\View\Element\Template
{
		...
		if($arrPin){
			...
			foreach($arrPin as $pin){
				...
				if(trim($pin['custom_text'])!=''){
					...
					$html .= '<div class="pin__popup pin__popup--'.$pin['position'].' pin__popup--fade pin__popup_text_content" style="width:'.($productImageWidth + 30).'px">';
					if($pinTitle!=''){
					...
					if($product = $this->getProductInfo($pin['text'])){
						...
						$html .= '<div class="pin__title '.$saleableClass.'">'.$product->getName().'</div>';
     ...
     */
    public function getSwatchRender(\Magento\Catalog\Model\Product $product)
    {
        return $this->getLayout()->createBlock("Magento\Swatches\Block\Product\Renderer\Listing\Configurable")->setTemplate("Magento_Swatches::product/listing/renderer.phtml")->setProduct($product)->toHtml();
    }


    public function getSwatchAttributeId(\Magento\Catalog\Model\Product $product)
    {
        $swatchArray = $this->swatchHelper->getSwatchAttributesAsArray($product);
    ...
    protected function getSwatchesRender()
    {
        return $this->getLayout()->getBlock('category.product.type.details.renderers.configurable');

    }

    public function getProductSwatches(\Magento\Catalog\Model\Product $product)
    {
        $swatchesRender = $this->getSwatchesRender();

        $swatches = '';
        if ($swatchesRender) {
            $swatches = $swatchesRender->render(
                \Magento\Swatches\Block\Product\Renderer\Configurable::SWATCH_RENDERER_TEMPLATE,
                $product,
                [
                    'include_container' => true,
                    'display_minimal_price' => true,
