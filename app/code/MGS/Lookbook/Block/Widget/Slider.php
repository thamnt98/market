...
class Slider extends \MGS\Lookbook\Block\AbstractLookbook implements \Magento\Widget\Block\BlockInterface
{
    ...
    public function __construct(
        ...
        parent::__construct($context, $productContext, $_helper, $swatchHelper, $_productCollectionFactory, $urlHelper, $data);
	...
	public function getNextIcon(){
		...
		if($value == ''){
			$value = 'lookbook/' . $this->_helper->getStoreConfig('lookbook/slider/next_image');
		}
		
		return $this->getControlImageUrl($value);
	}
	...
	public function getPrevIcon(){
		...
		if($value == ''){
			$value = 'lookbook/' . $this->_helper->getStoreConfig('lookbook/slider/prev_image');
		}
