                ...
                foreach ($importData as $_module) {
                    if (isset($_module['tables'])) {
                        ...
                        foreach ($tables as $tablename => $rows) {
                            ...
                            if ((false !== strpos($table_name, "cms_page")) || (false !== strpos($table_name,
                                        "cms_block"))) {
                                $exist = true;
                            }
                            if ($overwrite) {
                            ...
                            foreach ($rows as $row) {
                                if ($exist) {
                                    $where = '';
                                    $query_data = $this->_vesImport->buildQueryImport($row, $table_name, $overwrite,
                                        $data['store_id']);
                            ...
                            if (isset($_config['key'])) {
                                ...
                                if ($result != $_config['value']) {
                                    if ((int)$data['store_id'] == 0) {
                                        $this->_configResource->saveConfig($_config['key'], $_config['value'],
                                            "default", (int)$data['store_id']);
                                    } else {
                                        $this->_configResource->saveConfig($_config['key'], $_config['value'], "stores",
                                            (int)$data['store_id']);
                                    }
                                }
                            }
