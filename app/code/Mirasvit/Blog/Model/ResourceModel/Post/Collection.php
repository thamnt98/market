...
class Collection extends AbstractCollection
{
    ...
    public function addCategoriesFilter(array $categoriesFilter)
    {
        foreach ($categoriesFilter as $conditionType => $values) {
            $categorySelect = $this->getConnection()->select()->from(
                ['cat' => $this->getTable('mst_blog_category_post')],
                'cat.post_id'
     ...
     *
     * @param array $tagsFilter
     * @return $this
     */
    public function addTagsFilter(array $tagsFilter)
    {
        foreach ($tagsFilter as $conditionType => $values) {
            $tagSelect = $this->getConnection()->select()->from(
                ['tag' => $this->getTable('mst_blog_tag_post')],
                'tag.post_id'
            )->where($this->getConnection()->prepareSqlCondition('tag.tag_id', ['in' => $values]));
            $selectCondition = [
                $this->mapConditionType($conditionType) => $tagSelect
            ];
            $this->getSelect()->where($this->getConnection()->prepareSqlCondition('e.entity_id', $selectCondition));
        }
        return $this;
